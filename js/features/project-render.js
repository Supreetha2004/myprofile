...
function renderProjects(projects) {
  ...
  projects.forEach((project, index) => {
    ...
    if (project.liveDemo && project.liveDemo !== "LiveDemo") {
      linksHTML.push(`
        <a href="${project.liveDemo}" target="_blank" class="proj-link">
    ...
    if (project.github) {
      linksHTML.push(`
        <a href="${project.github}" target="_blank" class="proj-link">
    ...
    card.innerHTML = `
      <div class="proj-top-row">
        <span class="proj-category-badge">${project.category}</span>
      </div>
      ...
      <div class="proj-tech-section">
        ...
        <div class="proj-tags">${techBadges}</div>
      </div>
      ${linksHTML.length > 0 ? `<div class="proj-links">${linksHTML.join('')}</div>` : ''}
    `;
